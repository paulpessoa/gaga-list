...
export default function PeoplePage() {
  ...
  const handleInviteByEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/push/nudge', { // Vou criar ou ajustar uma rota para convite
        method: 'POST',
      ...
      alert(`Convite enviado para ${inviteEmail}! Você ganhará 50 grãos quando ele se cadastrar.`)
      setInviteEmail("")
    } catch (err) {
  ...
  const handleAddToSelectedList = async (listId: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(`${selectedFriend.full_name || selectedFriend.email} foi adicionado à lista!`)
      }
    } catch (err) {
      ...
      <section className="glass-panel p-8 rounded-[2.5rem] bg-indigo-500/5 border-2 border-indigo-500/10 flex flex-col gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-tight">Convidar Amigos</h2>
          </div>
          <p className="text-sm text-zinc-600 dark:text-zinc-400 font-medium mb-6 leading-relaxed">
            Convide alguém para o app. Quando eles entrarem, você ganha <strong>50 grãos mágicos</strong>! 🌾
          </p>
          <form onSubmit={handleInviteByEmail} className="flex flex-col sm:flex-row gap-3">
            ...
            >
              {isInviting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Convidar e Ganhar"}
            </button>
          </form>
        </div>
      </section>
        ...
        <section className="space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="py-20 text-center space-y-4 bg-zinc-50 dark:bg-zinc-900/20 rounded-[3rem] border-2 border-dashed border-zinc-100 dark:border-white/5">
              ...
              <p className="text-zinc-500 text-sm font-medium">Nenhum contato encontrado.</p>
            </div>
          ) : (
