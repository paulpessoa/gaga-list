...
test.describe("Gaga List - E2E Flow Validation", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const loginButton = page
      .locator('button:has-text("Entrar"), a:has-text("Entrar")')
      .first()
  ...
  test("Flow: Notifications & Empty State", async ({ page }) => {
    ...
    if (await page.locator('h2:has-text("Tudo limpo por aqui")').isVisible()) {
      await expect(
        page.locator('button:has-text("Copiar Link de Convite")')
      ).toBeVisible()
    }
  })
  ...
  test("Flow: Profile & Credits", async ({ page }) => {
    ...
    await expect(
      page.locator('h2:has-text("O que são Grãos Mágicos?")')
    ).toBeVisible()
  })
})
