...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (audioBlob && !isRecording) { handleProcessVoice(audioBlob); setAudioBlob(null); }
  }, [audioBlob, isRecording, handleProcessVoice, setAudioBlob])

  const handleCreateList = () => { trigger("medium"); setIsCreateModalOpen(true); }
  const submitCreateList = (e: React.FormEvent) => {
    e.preventDefault(); if (!newListTitle.trim()) return;
    createList.mutate({ title: newListTitle, color_theme: "indigo" }, { onSuccess: () => { setIsCreateModalOpen(false); setNewListTitle(""); } });
  }
  const handleRename = (listId: string, currentTitle: string) => { setEditingListId(listId); setEditTitle(currentTitle); }
  const submitRename = (listId: string) => {
    if (!editTitle.trim()) { setEditingListId(null); return; }
    updateList.mutate({ listId, updates: { title: editTitle } }, { onSuccess: () => setEditingListId(null) });
  }
  const handleScanSuccess = (decodedText: string) => {
    trigger("success" as any); setIsQrScannerOpen(false);
    try { if (decodedText.includes('/join/')) { const url = new URL(decodedText); router.push(url.pathname); } else { alert("QR Inválido."); } } catch (e) { alert("Erro no QR."); }
  };
    ...
    <main className="min-h-screen p-6 md:p-12 max-w-5xl mx-auto flex flex-col gap-10 pb-32 bg-white dark:bg-zinc-950 transition-colors duration-300">
      <header className="flex items-start justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-black tracking-tight text-zinc-900 dark:text-white leading-tight">Minhas Listas</h1>
          <p className="text-sm text-zinc-500 font-medium">{lists?.length || 0} listas ativas • Gerencie seus itens</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <button onClick={handleCreateList} disabled={createList.isPending} className="glass-panel rounded-[2rem] p-8 flex flex-col items-center justify-center gap-4 min-h-[200px] border-dashed border-2 border-zinc-200 dark:border-white/5 bg-zinc-50/50 dark:bg-zinc-900/20 hover:bg-zinc-100 dark:hover:bg-zinc-900/40 transition-all group cursor-pointer">
          ...
          <div className="flex flex-col items-center">
            <span className="text-base font-bold text-zinc-900 dark:text-zinc-100">{createList.isPending ? "Criando..." : "Nova Lista"}</span>
        ...
        {isLoading ? ([1, 2].map(i => (
          <div key={i} className="glass-panel rounded-[2rem] p-8 min-h-[200px] animate-pulse">
            <div className="flex gap-4 mb-6"><div className="w-12 h-12 bg-zinc-200 dark:bg-zinc-800 rounded-2xl" /><div className="flex-1 space-y-2"><div className="h-5 bg-zinc-200 dark:bg-zinc-800 rounded w-1/2" /><div className="h-3 bg-zinc-200 dark:bg-zinc-800 rounded w-1/3" /></div></div>
          ...
          <div className="col-span-full glass-panel rounded-[2rem] p-12 flex flex-col items-center justify-center border-red-500/20 gap-4 text-center">
            <div className="w-16 h-16 rounded-full bg-red-500/10 flex items-center justify-center"><X className="w-8 h-8 text-red-500" /></div>
          ...
          lists?.map((list: any) => {
            ...
            return (
              <div key={list.id} onClick={() => { trigger("light"); router.push(`/dashboard/lists/${list.id}`); }} className="glass-panel card-hover rounded-[2rem] p-6 flex flex-col justify-between min-h-[200px] cursor-pointer border-zinc-100 dark:border-white/5 bg-white dark:bg-zinc-900/40 relative overflow-hidden group">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4 overflow-hidden">
                    <div className="w-12 h-12 shrink-0 rounded-2xl bg-indigo-500/10 dark:bg-indigo-500/20 flex items-center justify-center text-2xl shadow-inner border border-indigo-500/10">{list.icon || "🛒"}</div>
                    <div className="flex flex-col min-w-0">
                      {editingListId === list.id ? (
                        <input autoFocus value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onBlur={() => submitRename(list.id)} onKeyDown={(e) => e.key === "Enter" && submitRename(list.id)} onClick={(e) => e.stopPropagation()} className="bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white font-bold text-lg rounded px-2 py-0.5 outline-none ring-2 ring-indigo-500" />
                      ) : (
                        <h3 className="font-black text-lg text-zinc-900 dark:text-white truncate group-hover:text-indigo-500 transition-colors leading-tight">{list.title}</h3>
                      )}
                      <div className="flex items-center gap-2 mt-0.5"><span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${isOwner ? "bg-indigo-500/10 text-indigo-600" : "bg-amber-500/10 text-amber-600"}`}>{isOwner ? "Proprietário" : "Colaborador"}</span></div>
                    </div>
                  </div>
                </div>
                <div className="mt-4">
                  <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest mb-2 text-zinc-400"><span>Progresso</span><span className="text-zinc-900 dark:text-zinc-100">{completedItems}/{totalItems} itens</span></div>
                  <div className="w-full h-2.5 bg-zinc-100 dark:bg-zinc-800/50 rounded-full overflow-hidden shadow-inner border border-zinc-200/50 dark:border-white/5"><div className="h-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.4)] transition-all duration-700 ease-out" style={{ width: `${progress}%` }} /></div>
                </div>
                <div className="mt-6 pt-4 border-t border-zinc-100 dark:border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <button onClick={(e) => { e.stopPropagation(); handleRename(list.id, list.title); }} className="p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 text-zinc-500 hover:text-indigo-500 hover:bg-white dark:hover:bg-zinc-700 transition-all border border-transparent hover:border-zinc-200 dark:hover:border-white/10"><Edit2 className="w-4 h-4" /></button>
                    {isOwner && (<button onClick={(e) => { e.stopPropagation(); if (confirm("Mover para a lixeira?")) deleteList.mutate(list.id); }} className="p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 text-zinc-500 hover:text-red-500 hover:bg-white dark:hover:bg-zinc-700 transition-all border border-transparent hover:border-zinc-200 dark:hover:border-white/10"><Trash2 className="w-4 h-4" /></button>)}
                  </div>
                  <div className="flex items-center gap-2 px-4 py-2 bg-indigo-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all">Acessar <ChevronRight className="w-3 h-3 ml-1" /></div>
                </div>
              </div>
            )
          })
