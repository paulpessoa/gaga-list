                ...
                <>
                  <div className="mb-8 text-center">
                    <div className="w-16 h-16 bg-[#1DB954]/10 dark:bg-[#1DB954]/20 rounded-3xl flex items-center justify-center mb-6 mx-auto">
                      <Plus className="w-8 h-8 text-[#53E076]" />
                    </div>
                  ...
                  <div className="flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <button
                        type="button"
                        onClick={() => checkAndAct(costs.cost_ocr, () => { trigger("medium"); setIsOcrScannerOpen(true); })}
                        disabled={isAiProcessing}
                        className="p-6 w-full bg-zinc-50 dark:bg-[#1c1b1b]/50 text-zinc-600 dark:text-zinc-400 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex flex-col items-center justify-center gap-3 hover:bg-white dark:hover:bg-zinc-900 transition-all border border-zinc-100 dark:border-[#3d4a3d]/60 active:scale-95 min-h-[140px] shadow-sm shadow-inner"
                      >
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full bg-zinc-100 dark:bg-[#1c1b1b] border-2 border-transparent focus:border-[#53E076]/50 rounded-2xl py-5 px-6 text-zinc-900 dark:text-white placeholder:text-zinc-400 dark:placeholder:text-zinc-700 focus:outline-none transition-all shadow-inner font-bold text-base"
                          />
                        </div>

                        {/* Sugestões inline */}
                        {suggestions.length > 0 && (
                          <div className="absolute top-full left-0 right-0 mt-3 z-30 bg-white dark:bg-[#1c1b1b] rounded-[1.5rem] shadow-2xl border border-zinc-100 dark:border-zinc-800 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                            {suggestions.map((s, i) => (
                              <button
                                key={i}
                                type="button"
                ...
                <div className="animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <div className="bg-zinc-50 dark:bg-[#1c1b1b] rounded-[2.5rem] p-6 max-h-80 overflow-y-auto border border-zinc-100 dark:border-[#3d4a3d]/60 shadow-inner mb-8">
                    <ul className="space-y-3">
                      {voiceItems.map((item, i) => (
                        <li
                          key={i}
                          className="flex items-center gap-3 text-sm font-bold text-zinc-700 dark:text-zinc-300 bg-white dark:bg-[#201f1f]/50 p-4 rounded-2xl border border-zinc-100 dark:border-[#3d4a3d]/60"
                        >
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      className="w-full py-5 bg-[#1DB954] text-white rounded-[1.5rem] font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all"
                    >
