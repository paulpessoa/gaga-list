      ...
      <div className="glass-panel w-full max-w-lg h-[85vh] sm:h-[600px] rounded-t-3xl sm:rounded-3xl flex flex-col relative shadow-2xl border border-white/10 animate-in slide-in-from-bottom sm:zoom-in-95 duration-300">
        <header className="p-6 border-b border-white/5 flex items-center justify-between bg-zinc-900/50 rounded-t-3xl">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center ${isIndividual ? "bg-amber-500/10 text-amber-500" : "bg-indigo-500/10 text-indigo-400"}`}
            >
              {isIndividual ? (
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div
                        className={`py-3 px-4 rounded-2xl text-sm ${isMine ? "bg-indigo-600 text-white rounded-tr-none shadow-lg shadow-indigo-500/10" : "bg-zinc-800 text-zinc-200 rounded-tl-none border border-white/5"}`}
                      >
                        {msg.content}
                      </div>
                      <span
                        className={`text-[9px] text-zinc-600 ${isMine ? "text-right mr-1" : "ml-1"}`}
                      >
        ...
        <footer className="p-4 bg-zinc-950/50 border-t border-white/5 sm:rounded-b-3xl">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              type="text"
              placeholder={
                isIndividual ? "Segredo efêmero..." : "Escreva uma mensagem..."
              }
              value={newMessage}
