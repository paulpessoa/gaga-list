    ...
    <main className="min-h-screen p-6 md:p-12 max-w-2xl mx-auto flex flex-col gap-10 pb-32 bg-white dark:bg-zinc-950 transition-colors duration-300">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 ml-1">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Identidade</h2>
        </div>
        <div className="glass-panel p-8 rounded-[2.5rem] bg-gradient-to-br from-indigo-500/5 to-purple-500/5 border-2 border-indigo-500/10 flex flex-col items-center text-center gap-4 relative overflow-hidden">
          <div className="w-24 h-24 rounded-[2rem] bg-indigo-500/10 border-4 border-white dark:border-zinc-900 shadow-2xl flex items-center justify-center relative group">
            {profile?.avatar_url ? (
              <img src={profile.avatar_url} alt={profile.full_name || ""} className="w-full h-full object-cover rounded-[1.8rem]" />
            ) : (
          ...
          <div>
            <h3 className="text-xl font-black text-zinc-900 dark:text-white">{profile?.full_name || "Usuário"}</h3>
            <p className="text-sm text-zinc-500 font-medium">{profile?.email || user?.email}</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 ml-1">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Energia IA</h2>
        </div>
        <Link 
          href="/app/credits"
          onClick={() => trigger('light')}
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-black text-zinc-900 dark:text-white">Meus Grãos Mágicos</p>
              <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest">{credits} disponíveis</p>
            </div>
          </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 ml-1">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Preferências do App</h2>
        </div>
        <div className="glass-panel rounded-[2.5rem] overflow-hidden border border-zinc-100 dark:border-white/5 bg-white dark:bg-zinc-900/40">
          ...
          <div className="p-6 flex items-center justify-between border-b border-zinc-50 dark:border-white/5">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-zinc-700 dark:text-zinc-300">Notificações Push</span>
            </div>
            <button 
              onClick={togglePush}
            ...
            >
              <div className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow-sm transition-all ${isPushEnabled ? 'left-7' : 'left-1'}`} />
            </button>
          </div>
          ...
          <div className="p-6 flex items-center justify-between opacity-50">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-zinc-700 dark:text-zinc-300">Modo Escuro (Auto)</span>
            </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 ml-1">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Ajuda</h2>
        </div>
        <div className="grid grid-cols-1 gap-3">
          <button className="glass-panel p-5 rounded-2xl flex items-center justify-between group hover:border-indigo-500/20 transition-all bg-white dark:bg-zinc-900/40 border border-zinc-100 dark:border-white/5">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-zinc-700 dark:text-zinc-300">Central de Ajuda</span>
            </div>
      ...
      <section className="mt-4 pt-10 border-t border-zinc-100 dark:border-white/5 space-y-4">
        ...
        <footer className="text-center space-y-1">
          <p className="text-[10px] text-zinc-400 dark:text-zinc-600 uppercase font-black tracking-widest">
            Lista Pronta v1.1.0 (Apple Standard)
          </p>
