...
export function LegacyVisionScanner({ isOpen, onClose, onScanSuccess, mode = 'product' }: VisionScannerProps) {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      alert('Permissão de câmera negada no motor antigo.');
      onClose();
    }
  }, [onClose]);
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black flex flex-col animate-in fade-in duration-300">
        ...
        {isProcessing && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-4 text-white p-8 text-center">
            ...
            <div className="space-y-1">
              <h3 className="text-xl font-black">Processando Antigo</h3>
              <p className="text-zinc-400 text-sm font-medium">Extraindo itens...</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-10 bg-zinc-950 flex items-center justify-center gap-8 relative">
        <button onClick={() => { setCapturedImage(null); setIsAiProcessing(false); startCamera(); }} className="p-4 rounded-full bg-zinc-900 text-zinc-500 active:scale-95 transition-all"><RefreshCw className="w-6 h-6" /></button>
        <button onClick={capturePhoto} disabled={!!capturedImage || isProcessing} className="w-20 h-20 rounded-full bg-white flex items-center justify-center shadow-[0_0_30px_rgba(255,255,255,0.3)] active:scale-90 transition-all disabled:opacity-50"><div className="w-16 h-16 rounded-full border-4 border-zinc-950 flex items-center justify-center"><div className="w-12 h-12 rounded-full bg-amber-500" /></div></button>
        <div className="w-14" />
      </div>
