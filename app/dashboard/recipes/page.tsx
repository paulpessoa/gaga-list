...
export default function RecipesPage() {
  ...
  const generateRecipes = async (type: 'from_list' | 'custom' | 'from_products') => {
    if (type === 'from_list') setIsLoadingList(true)
    else if (type === 'custom') setIsLoadingCustom(true)
    else setIsLoadingProducts(true)
    
    setLoadingStatus(
      type === 'from_products' ? "Chef IA analisando seu catálogo..." :
      type === 'from_list' ? "Consultando sua lista de compras..." : "Preparando sugestões criativas..."
    )
  ...
  return (
    <main className="min-h-screen p-6 md:p-12 max-w-5xl mx-auto flex flex-col gap-10 pb-32 bg-white dark:bg-zinc-950 transition-colors duration-300">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-zinc-500 font-medium">Crie pratos incríveis com IA baseados no que você tem</p>
      </header>
        ...
        <div className="md:col-span-2 glass-panel p-8 rounded-[2.5rem] flex flex-col gap-6 bg-emerald-500/5 border-emerald-500/10 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 text-emerald-500">
              ...
              <h2 className="font-black uppercase tracking-widest text-xs">Receita com Meus Produtos</h2>
            </div>
          ...
          <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1 custom-scrollbar">
            ...
            {myProducts.length === 0 && (
              <Link href="/dashboard/products" className="text-[10px] font-bold text-zinc-400 py-4 hover:text-emerald-500 underline">Você ainda não tem produtos salvos. Scaneie produtos para começar.</Link>
            )}
          </div>
          ...
          >
            {isLoadingProducts ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : `Criar Receita com ${selectedProducts.length} itens`}
          </button>
        </div>
      </div>
      ...
      {loadingStatus && (
        <div className="flex flex-col items-center justify-center py-10 gap-4 animate-pulse">
           ...
           <p className="text-sm font-black text-indigo-500 uppercase tracking-widest">{loadingStatus}</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-t-[2.5rem] sm:rounded-[3rem] shadow-2xl animate-in slide-in-from-bottom duration-500">
            <div className="sticky top-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-6 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between z-10">
               <button onClick={() => setViewingRecipe(null)} className="p-2 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors text-zinc-500">
                 <X className="w-6 h-6" />
               </button>
               <h3 className="text-sm font-black uppercase tracking-widest text-zinc-900 dark:text-white">Receita Salva</h3>
               <button onClick={() => deleteSavedRecipe(viewingRecipe.id)} className="p-2 rounded-full hover:bg-rose-50 dark:hover:bg-rose-500/10 text-zinc-400 hover:text-rose-500 transition-colors">
            ...
            <div className="p-8">
              <div className="flex items-center gap-3 mb-4">
                <span className="px-3 py-1 bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-lg text-[10px] font-black uppercase tracking-widest">{viewingRecipe.difficulty}</span>
                <span className="text-zinc-400 text-[10px] font-bold uppercase flex items-center gap-1"><Clock className="w-3 h-3" /> {viewingRecipe.prep_time}</span>
              </div>
              
              <h2 className="text-3xl font-black text-zinc-900 dark:text-white mb-4">{viewingRecipe.title}</h2>
              <p className="text-zinc-500 font-medium mb-10 leading-relaxed">{viewingRecipe.description}</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                 <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-6">Ingredientes</h4>
                 ...
                 <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-6">Passo a Passo</h4>
                    <ol className="space-y-4">
                       {viewingRecipe.instructions.map((step: string, i: number) => (
                         <li key={i} className="flex gap-3 text-sm font-medium text-zinc-500 dark:text-zinc-400 leading-relaxed">
                            <span className="font-black text-indigo-500">{i+1}.</span>
                            {step}
                         </li>
                       ))}
